...
SQL Analysis
SQL was used for database creation, table creation, data inspection, data validation, record-count validation, duplicate checking, null/value checking, KPI validation, and cross-table business analysis.
•	Order volume
•	Sales and revenue
•	Product performance
...
•	Management Reporting – Provide a centralized dashboard for faster, data-driven decisions.
Project Workflow
Raw Dataset → Data Inspection → Data Cleaning & Validation → MySQL Database Creation → Table Creation → SQL Data Validation → SQL Analysis → Power BI Data Connection → Power Query Transformation → Data Modelling & Relationships → DAX Measures & KPIs → Dashboard Development → Business Insights → Recommendations
